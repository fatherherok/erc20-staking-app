...
contract Staking {
    ...
    uint public currentTokenId = 1; // for the different token to stake
...
// for token metadata
    ...
    //for address that do the staking
    struct Position {
        uint positionId;
    ...
    uint public ethUsdPrice; //current price

     string[] public tokenSymbols; //to store token sysmbol or tokens added
...
function closePosition(uint positionId) external  {
    ...
    uint weiAmount = calculateInterest(positions[positionId].apy, 
